...
class mynet(nn.Module):
    def __init__(self, device, num_nodes=320, in_dim=1, supports=None):
        ...
        self.mem_rep = Memory(memory_size=10, feature_dim=in_dim, key_dim=in_dim, temp_update=0.1, temp_gather=0.1)
        self.decoder = Decoder(in_dim=512, hidden_dim=512, out_dim=12)

    def forward(self, input, memory, train=True):
        batch_size, c_in, n_node, seq_len = input.shape

        hidden = self.encoder(input)
        res_mem, memory, gathering_loss, spreading_loss = self.mem_rep(hidden, memory, train)
        hidden = torch.cat((hidden, res_mem), dim=1)
        output = self.decoder(hidden)
        if train:
            return output, memory, gathering_loss, spreading_loss
        else:
            return output
